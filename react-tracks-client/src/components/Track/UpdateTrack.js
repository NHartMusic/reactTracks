...
const UpdateTrack = ({ classes, track }) => {
  ...
  const handleAudioChange = event => {
    ...
    if (selectedFile && selectedFile.size > fileSizeLimit) {
      setFileError(`${selectedFile.name}: File is too large bruh, u should know this shit by now`)
    } else {
  ...
  const handleAudioUpload = async () => {
    try {
      ...
      data.append('cloud_name', 'nhartmusic')
      const res = await axios.post('https://api.cloudinary.com/v1_1/nhartmusic/raw/upload', data)
      return res.data.url
    } catch (err) {
  ...
  const handleSubmit = async (event, updateTrack) => {
    ...
    //upload audio + get returned url from API 
